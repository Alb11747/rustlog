    ...
    async fn run_init_task(
        ...
        let res = async {
            ...
            let rate_limit_permit = Arc::clone(&config.connection_rate_limiter)
                .acquire_owned()
                .await
                .expect("connection rate limiter was unexpectedly closed");
            tracing::trace!("Successfully got permit to open transport.");

            // Start the cooldown before attempting the transport. The upstream implementation
            // did this only after a successful connection, so DNS and transport failures dropped
            // the permit immediately and bypassed new_connection_every.
            release_rate_limit_permit_after(rate_limit_permit, config.new_connection_every);
...
fn release_rate_limit_permit_after(permit: OwnedSemaphorePermit, delay: Duration) {
    tokio::spawn(
        async move {
            tokio::time::sleep(delay).await;
            drop(permit);
            tracing::trace!("Successfully released permit after waiting specified duration.");
        }
...
#[cfg(test)]
mod tests {
    use super::release_rate_limit_permit_after;
    use std::{sync::Arc, time::Duration};
    use tokio::{sync::Semaphore, time::timeout};

    #[tokio::test]
    async fn rate_limit_permit_is_held_for_the_full_cooldown() {
        let semaphore = Arc::new(Semaphore::new(1));
        let permit = semaphore.clone().acquire_owned().await.unwrap();
        release_rate_limit_permit_after(permit, Duration::from_millis(50));

        assert!(semaphore.try_acquire().is_err());
        assert!(timeout(Duration::from_millis(10), semaphore.acquire())
            .await
            .is_err());
        let _permit = timeout(Duration::from_millis(250), semaphore.acquire())
            .await
            .expect("permit was not released after the cooldown")
            .unwrap();
    }
}
